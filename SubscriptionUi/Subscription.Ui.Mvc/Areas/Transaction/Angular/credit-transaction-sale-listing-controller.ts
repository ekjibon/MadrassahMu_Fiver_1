class creditTransactionSaleListingController {
    scope;
    ...
    public calculateRemaining(item: transactionModel) : number {
        var self = this;
        console.log(item)
        var amountPaid = item.totalAmount;
        var totalAmount = 0;
        item.transactionDetails.forEach((td) => {
...
transactionModule.controller("creditTransactionSaleListingController"
    , ["$scope"
        , "transactionSaleWebService"
