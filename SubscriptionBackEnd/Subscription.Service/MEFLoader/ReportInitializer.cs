...
{
    public class ReportInitializer<T>
    {
        [ImportMany(typeof(IReportGenerator<>))]
        public IEnumerable<Lazy<IReportGenerator<T>, IReportGeneratorMetadata>> reports; //Lazy import ensures components are loaded only when required
